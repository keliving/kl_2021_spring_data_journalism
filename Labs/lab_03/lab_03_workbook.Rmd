...
```{r}
#codeblock to glimpse death rate data
...
# A: Prince George's County had the highest case rate with 29.27 cases per 1,000.
...
# A: Bath County, Virginia had the lowest case rate on August 30, with .91 cases per 1,000. 
...
# A: Only two areas had death rates above 4 per 1,000 in August: Hancock County, Georgia and Galax City, Virginia. 
...
# A: Only Kent County, Maryland had a death rate above 1 per 1,000 in August. 
...
# A: Galax City, Emporia City and Northampton County, Virginia all had death rates higher than 2 per 1,000 in August. 
...
# A: Lafayette County, Florida had the biggest increase in cumulative case rate between July and August. 

```

The new column we created, jul_aug_change, calculated absolute difference.  

But let's suppose we want to calculate percentage change, to answer this slightly different question: "Which county had the biggest percentage jump in cumulative case rate between July and August?"
...
# A: Luce County, Michigan had the biggest percentage jump in its cumulative case rate between July and August. 
...
covid_maryland_jun_aug_percent_change <- covid_county_monthly_death_rate %>%
...
covid_texas_may_aug_pct_change <- covid_county_monthly_case_rate %>%
  ...
  filter(state == "Texas") %>%
  mutate(may_aug_pct_change = (d_2020_08_30 - d_2020_05_30)/d_2020_05_30*100) %>%
  arrange(may_aug_pct_change)

# Jones County, Texas had the smallest percent change in cumulative case rate between May and August at 49.13%.
...
```{r}
#Question: What county in Florida had the highest percent change in death rate between May and August?

covid_florida_may_aug_death_change <- covid_county_monthly_death_rate %>%
  select(county, state, d_2020_05_30, d_2020_08_30) %>%
  filter(state == "Florida") %>%
  mutate(may_aug_pct_change = (d_2020_08_30 - d_2020_05_30)/d_2020_05_30*100) %>%
  arrange(desc(may_aug_pct_change))

#Jackson County, Florida had the highest percent change in death rate between May and August at 6,500%.
