...
# Q: How many cases did the average U.S. county have on August 30?
# A: 1,907.15, or about 1,907 cases
...
# A: Vermont had the fewest total covid cases on August 30, with 1,608 cases. 
```
Let's answer another, similar question. 
...
# A: Alaska had the fewest total covid deaths on August 30, with 34 deaths. 
...
# Write the answer in a comment
# Q: What was the average number of cases per county in each state? Which state had the lowest average?
# A: Vermont had the lowest average number of cases per county at 114.86, or about 115 cases. 
...
# Write the answer in a comment
# Q: Which state has the most counties in our data? How many?
# A: Texas has the most counties, with 251. 
```
...
counties_per_state_ascending <- covid_county %>%
  group_by(state) %>%
  summarise(total_counties = n() ) %>%
...
counties_per_state_ascending

# Q: which state has the smallest number of counties? 
# A: The data would say the District of Columbia, but that is not technically a state. Delaware is the state with the smallest number of counties. 
...
```{r}
# What was the average number of covid cases per county in Maryland?  

maryland_average_county_cases <- covid_county %>%
  group_by(state) %>%
  filter(state == "Maryland") %>%
  summarise(maryland_average_county_cases = mean(cases))

maryland_average_county_cases

# The average number of cases per county in Maryland was 4,511.92, or about 4,512 cases. 
