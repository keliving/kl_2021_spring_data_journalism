...
census_tract_stats <- get_acs(geography = "tract", variables = c( "B01001_001","B02001_002","B02001_003","B02001_004","B03001_003","B06012_002","B19013_001"), state=states,year = 2018) %>%
  ...
  clean_names() %>%
  select(geoid, total_pop, starts_with("pct"),median_income)
...
safegraph_match<-read_rds("data/safegraph_match.rds")
...
* **Analysis summary**: For Texas, as noted by the tx_demos data set, we noticed that bucket 1, the most white areas, showed the highest concentration of purchases, with an average per capita spending of $80.90 for per_capita_20. 

For Connecticut, as noted by the ct_retail_demo data set, we noticed that bucket 3, the areas with non-white populations falling in the middle 20%, showed the highest concentration of purchases, with an average per capita spending of $80.43 for per_capita_20. 

For Missouri, as noted by the mo_demos data set, we noticed that bucket 5, areas with the highest non-white population, showed the ihghest concentration of purchases, with an average per capita spending of $83.26 for per_capita_20. 

For Texas, as noted by the tx_money data set, we noticed that bucket 3, which features the median income level, showed the highest concentration of purchases, with an average per capita spending of $63.41 for per_capita_20. 

For Connecticut, as noted by the ct_money data set, we noticed that bucket 1, which features the lower income level, showed the highest concentration of purchases, with an average per capita spending of $74.99 for per_capita_20. 

For Missouri, as noted by the mo_money data set, we noticed that bucket 1, which features the lowest income level, showed the highest concentration of purchases, with an average per capita spending of $60.78 for per_capita_20 and $6.90 for per_capita_21. 
...
#Think about race and median income as separate questions for now
...
* **Analysis summary**: For Texas, when referencing the tx_money_over_time data set, we noticed that the lowest and second-lowest income tiers (buckets 1 and 2) showed the most significant bumps in lottery purchases between 2019 and 2020 (16.83% and 16.91%, respectively). 

For Connecticut, when referencing the ct_money_over_time data set, we noticed that the second lowest income level and the lowest level income tiers (buckets 2 and 1) showed the most significant bumps in lottery purchases between 2019 and 2020 (7.102% and 5.422%, respectively). 

For Missouri, when referencing the mo_money_over_time data set, we noticed that the lowest income level and the second lowest income level income tiers (buckets 1 and 2) showed the most significant bumps in lottery purchases between 2019 and 2020 (18.15% and 15.12%, respectively). 
...
ct_money_over_time <- ct_money%>%
 ...
 # select(!count)%>%
  #mutate
  #mutate(non_white_bucket=as.character(non_white_bucket))%>%
  #pivot_longer(!non_white_bucket,names_to = "sales_year",values_to = "sales")%>%
  #mutate(bucket_1=case_when(
   # non_white_bucket=="1" ~ sales,
    #TRUE ~ 0
  #))%>%
#mutate(bucket_2=case_when(
 #   non_white_bucket=="2" ~ sales,
  #  TRUE ~ 0
  #))%>%
  #mutate(bucket_3=case_when(
   # non_white_bucket=="3" ~ sales,
    #TRUE ~ 0
  #))%>%
  #mutate(bucket_4=case_when(
   # non_white_bucket=="4" ~ sales,
    #TRUE ~ 0
  #))%>%
  #mutate(bucket_5=case_when(
   # non_white_bucket=="5" ~ sales,
    #TRUE ~ 0
  #))%>% 
  #group_by(sales_year)
